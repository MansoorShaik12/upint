...
const { Contacts, ContactHistory } = require('./models/contacts')
...
app.post('/candidate', async (req, res) => {
  try {
    const { FirstName, LastName, Email, Phone, Date_Of_Birth, Gender, HigherQualification, UniversityCollege, CurrentExperience, skills, Position, PositionId, OwnerId, orgId } = req.body;
...
app.put('/candidate/:id', async (req, res) => {
  ...
  const { FirstName, LastName, Email, Phone, Date_Of_Birth, Gender, HigherQualification, UniversityCollege, CurrentExperience, skills, Position, PositionId, OwnerId } = req.body;
  try {
...
app.post('/position', async (req, res) => {
  const { title, companyname, jobdescription, minexperience, maxexperience, date, skills, additionalnotes, rounds, OwnerId, orgId } = req.body;
  if (!OwnerId) {
    return res.status(400).json({ error: "OwnerId field is required" });
  }
...
app.put('/team/:id', async (req, res) => {
  const { _id, FirstName, LastName, Email, Phone, CompanyName, Technology, Location, CurrentRole, skills, OwnerId } = req.body;
  if (_id) {
    try {
...
app.post('/assessment', async (req, res) => {
  try {
    ...
    const assessments = await Assessment.find({ OwnerId: req.body.OwnerId });
...
const addTabsData = async () => {
  ...
  try {
    ...
    console.log('Tabs data added successfully');
  } catch (err) {
...
const addObjectsData = async () => {
  ...
  try {
    ...
    console.log('objects data added successfully');
  } catch (err) {
...
const addSharingRulesObjectData = async () => {
  ...
  try {
    ...
    console.log('SharingRulesObject data added successfully');
  } catch (err) {
...
// route for checking existing email
// app.post('/organization/check-email', async (req, res) => {
...
app.put('/organization/user/:userId', async (req, res) => {
  const { firstName, lastName, email, phone, username, jobTitle, company, employees, country, password } = req.body;
  ...
  try {
    ...
    const contact = await OrganizationContact.findByIdAndUpdate(
    ...
    const user = await OrganizationUser.findByIdAndUpdate(
      organization.user,
    ...
    res.status(200).json({ organization: updatedOrganization, user, contact });
  } catch (error) {
...
app.get('/api/:model', async (req, res) => {
  ...
  console.log(`Received request for ${model} with:`, { orgId, OwnerId });
...
app.get('/api/organization/:model', async (req, res) => {
  ...
  console.log(`Received request for ${model} with organizationId: ${organizationId}`);
...
app.get('/api/outsource-interview', async (req, res) => {
  try {
    ...
    console.log('Sending outsource interviews:', interviews);
    res.json(interviews);
  } catch (error) {
...
const addInitialPlansData = async () => {
  ...
  try {
    ...
    console.log('Plans data added successfully');
  } catch (err) {
    console.error('Error adding plans data:', err);
  }
};

// Uncomment the following line to add initial plans data
// addInitialPlansData();



// Call the function to add tabs data
