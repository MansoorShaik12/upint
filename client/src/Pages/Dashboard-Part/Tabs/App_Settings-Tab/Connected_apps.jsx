...
import Sidebar from "./Connected_apps_form.jsx";
...
const Connected_apps = () => {
    ...
    useEffect(() => {
        const fetchConnectedApps = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/connected-apps`);
    ...
    const filteredApps = connectedApps.filter(app =>
        ...
        app.redirectUrls.toLowerCase().includes(searchQuery.toLowerCase()) ||
        app.scope.some(scope => scope.toLowerCase().includes(searchQuery.toLowerCase()))
    );
    ...
    const currentFilteredRows = filteredApps.slice(startIndex, endIndex);
    ...
    const handleCandidateClick = async (app) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/connected-apps/${app._id}`);
            setSelectedAppData(response.data);
    ...
    const handleRefreshToken = async () => {
        try {
            const response = await axios.put(`${process.env.REACT_APP_API_URL}/connected-apps/${selectedAppData._id}/refresh-token`);
            const { accessToken, expiry } = response.data;
            setSelectedAppData(prevData => ({
                ...prevData,
                accessToken,
                expiry
            }));
                ...
                <div>
                    <div className="fixed top-24 left-64 right-0">
                        <div className="flex justify-between p-4">
                            <div>
                                <span className="text-lg font-semibold">Connected Apps</span>
                            </div>
                                                ...
                                                <div className="overflow-y-auto min-h-80 max-h-96">
                                                    <table className="text-left w-full border-collapse border-gray-300 mb-14">
                                                        <thead className="bg-gray-300 sticky top-0 z-10 text-xs">
                                                            <tr>
                                                                <th scope="col" className="py-3 px-6">App Name</th>
                                                                <th scope="col" className="py-3 px-6">Redirect URL</th>
                                                                <th scope="col" className="py-3 px-6">Scopes</th>
                                                        ...
                                                        <tbody>
                                                            {loading ? (
                                                                ...
                                                                connectedApps.map((app) => (
                                                                    <tr key={app._id} className="bg-white border-b cursor-pointer text-xs">
                                                                        <td className="py-2 px-6 text-blue-400">
                                                                            <div className="flex items-center gap-3" onClick={() => handleCandidateClick(app)}>
                                                                                {app.appName}
                                                                            </div>
                                                                        </td>
                                                                        <td className="py-2 px-6">{app.redirectUrls}</td>
                                                                        <td className="py-2 px-6">{app.scope.join(', ')}</td>
                                                                        <td className="py-2 px-6 relative">
                                                                            <button onClick={() => toggleAction(app._id)}>
                                                                                <MdMoreHoriz className="text-3xl" />
                                                                            </button>
                                                                            {actionViewMore === app._id && (
                                                                                <div className="absolute z-10 w-36 rounded-md shadow-lg bg-white ring-1 p-4 ring-black ring-opacity-5 right-2 popup">
                                                                                    <div className="space-y-1">
                                                                                        ...
                                                                                        <p className="hover:bg-gray-200 p-1 rounded pl-3" onClick={() => handleEditClick(app)}>Edit</p>
                                                                                        <p className="hover:bg-gray-200 p-1 rounded pl-3" onClick={() => handlePopupClick(app.LastName)}>
                                                                                            Schedule
                                                                                        </p>
                                                                                    </div>
                                                                                </div>
                                                                        ...
                                                                        <div className="flex">
                                                                            ...
                                                                            <div className="flex flex-col">
                                                                                ...
                                                                                <div className="text-xs grid grid-cols-2 gap-1 items-start">
                                                                                    ...
                                                                                    <div className="text-gray-400">Client Secret</div>
                                                                                    <div>{candidate.Phone}</div>
                                                                                    <div className="text-gray-400">Redirect URL</div>
                ...
                <div>
                    <div className="fixed top-24 left-64 right-0">
                        <div className="flex justify-between items-center p-4 border-b">
                            <div className="flex items-center">
                                <span onClick={handleCloseProfile} className="cursor-pointer text-2xl mr-5"><MdOutlineKeyboardBackspace /></span>
                                <span className="text-lg font-semibold">Connected Apps</span>
                            </div>
                        </div>
                        {selectedAppData && (
                            <div className="mt-4 space-y-5 ml-10">
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">App Name :</p>
                                    <p>{selectedAppData.appName}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Description :</p>
                                    <p>{selectedAppData.description}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Origin URLs :</p>
                                    <p>{selectedAppData.originUrls}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Redirect URLs :</p>
                                    <p>{selectedAppData.redirectUrls}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Scope :</p>
                                    <p>{selectedAppData.scope?.join(', ')}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Client ID :</p>
                                    <p>{selectedAppData.clientId}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Client Secret:</p>
                                    <p>{selectedAppData.clientSecret}</p>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Access Token :</p>
                                    <p>{selectedAppData.accessToken}</p>
                                    <Tooltip title="Generate a new token" enterDelay={300} leaveDelay={100} arrow>
                                        <span onClick={handleRefreshToken} className="cursor-pointer ml-4 border-2 rounded-md p-1">
                                            Refresh Token
                                        </span>
                                    </Tooltip>
                                </div>
                                <div className="flex items-center">
                                    <p className="text-lg font-semibold w-48">Expiry :</p>
                                    <p>{new Date(selectedAppData.expiry).toLocaleString()}</p>
                                </div>
                            </div>
                        )}
                    </div>
