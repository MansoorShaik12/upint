...
const TaskForm = ({ isOpen, onClose, onTaskAdded, sharingPermissions }) => {
  ...
  const getOptionsForSelectedCategory = () => {
    switch (selectedCategoryRelatedTo) {
      case 'Candidates':
        return candidates.map(candidate => ({ name: candidate.LastName, id: candidate._id }));
      case 'Positions':
        return positions.map(position => ({ name: position.title, id: position._id }));
      case 'Teams':
        return teams.map(team => ({ name: team.LastName, id: team._id }));
      case 'Assessments':
        return assessments.map(assessment => ({ name: assessment.title, id: assessment._id }));
      case 'QuestionBank':
        return questionBanks.map(qb => ({ name: qb.title, id: qb._id }));
      case 'Interviews':
        return interviews.map(interview => ({ name: interview.title, id: interview._id }));
      case 'MockInterviews':
        return mockInterviews.map(mock => ({ name: mock.title, id: mock._id }));
      case 'Analytics':
        return analytics.map(analytic => ({ name: analytic.name, id: analytic._id }));
      default:
        return [];
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.relatedTo.objectName || !formData.relatedTo.recordId) {
      newErrors.relatedTo = "Related To is required";
    } else if (!/^[0-9a-fA-F]{24}$/.test(formData.relatedTo.recordId)) {
      newErrors.relatedTo = "Invalid recordId format";
    }
    if (!formData.dueDate) newErrors.dueDate = "Due Date is required";

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      console.log("Form validation failed:", newErrors); // Debugging log
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/tasks`, {
        ...
        relatedTo: {
          ...
          recordId: formData.relatedTo.recordId, // Use the stored ID
        },
      });
                ...
                <div className="flex-grow flex gap-4">
                  <div className="relative w-1/2">
                    <input
                      ...
                      readOnly
                      className={`border-b focus:outline-none mb-5 w-full ${errors.relatedTo ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                    />
                    <MdArrowDropDown
                      className="absolute right-0 top-1/2 transform -translate-y-1/2 cursor-pointer -mt-2"
                      onClick={toggleDropdownCategoryRelatedTo}
                      ...
                      <div className="absolute z-50 -mt-4 w-full rounded-md bg-white shadow-lg overflow-y-auto max-h-60">
                        {categoriesRelatedTo.map((category) => (
                          <div
                            key={category}
                            className="py-2 px-4 cursor-pointer hover:bg-gray-100"
                            onClick={() => handleCategorySelectRelatedTo(category)}
                          >
                            {category}
                          </div>
                        ))}
                  ...
                  <div className="relative w-1/2">
                    <input
                      ...
                      onClick={() => setShowDropdownOptionRelatedTo(!showDropdownOptionRelatedTo)}
                      readOnly
                    ...
                    {showDropdownOptionRelatedTo && (
                      <div className="absolute z-50 -mt-4 w-full rounded-md bg-white shadow-lg">
                        {getOptionsForSelectedCategory().map((option, index) => (
                          <div
                            key={option.id} // Use the ID as the key
                            className="py-2 px-4 cursor-pointer hover:bg-gray-100"
                            onClick={() => handleOptionSelectRelatedTo(option.name, option.id)} // Pass both name and id
                          >
                            {option.name}
                          </div>
                        ))}
