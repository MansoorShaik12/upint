...
import { MdMoreVert, MdOutlineContactPage, MdOutlineSchedule } from "react-icons/md";
...
const Home = ({ sharingPermissions, objectPermissions }) => {
  ...
  const apps = [
    ...
    {
      to: "", icon: <ImProfile className="text-4xl" />,
      title: "Profile", description: "Manage profile easily with our profile app and add new profile easily"
    },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (sortedCandidates.length > 0 && sortedCandidates[0].createdAt !== lastFetchedCandidates[0]?.createdAt) {
          ...
          setLastFetchedCandidates(sortedCandidates); // Move this inside the condition
        }
        ...
        if (sortedPositions.length > 0 && sortedPositions[0].createdAt !== lastFetchedPositions[0]?.createdAt) {
          ...
          setLastFetchedPositions(sortedPositions); // Move this inside the condition
        }
        ...
        if (sortedTeams.length > 0 && sortedTeams[0].createdAt !== lastFetchedTeams[0]?.createdAt) {
          ...
          setLastFetchedTeams(sortedTeams); // Move this inside the condition
        }

        // const sortedInterviews = interviews.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        // if (sortedInterviews.length > 0 && sortedInterviews[0].createdAt !== lastFetchedInterviews[0]?.createdAt) {
        //   setInterviewData(sortedInterviews);
        //   setLastFetchedInterviews(sortedInterviews); // Move this inside the condition
        // }
        const sortedInterviews = interviews.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    console.log(sharingPermissions.interviews, "sharingPermissions.interviews", sharingPermissions.candidate, "sharingPermissions.candidate", sharingPermissions.position, "sharingPermissions.position", sharingPermissions.team, "sharingPermissions.team");

    fetchData();
  }, [sharingPermissions]); // Remove other dependencies
  ...
  //   Math.max(notificationsData.length - 3, 0)
  // );
  ...
  const sortedInterviews = upcomingInterviews
    .filter(interview => {
      ...
      // Include interviews that are today and in the future, or any future date
      if (interviewDate) {
        if (interviewDate > currentDate) {
          return true; // Future date
        } else if (interviewDate.toDateString() === currentDate.toDateString()) {
          return interviewTime && interviewTime > currentDate; // Today's date but future time
        }
      }
      return false;
    })
  ...
  const fetchInterviewData = async () => {
    ...
    try {
      ...
      const interviewsWithCandidates = await Promise.all(allInterviews.map(async (interview) => {
        try {
      ...
      console.log(interviewsWithCandidates, "interviewsWithCandidates");
      const filteredInterviews = interviewsWithCandidates.filter(interview => interview.Interviewstype === '/outsourceinterview');
      setOutsourceInterviews(filteredInterviews);
    } catch (error) {
  ...
  const [filterCriteria, setFilterCriteria] = useState('all'); // New state for filter criteria

  const handleFilterChange = (criteria) => {
    setFilterCriteria(criteria);
    setOpenPopup(null); // Close the filter popup after selection
  };
  ...
  const filteredInterviews = sortedInterviews.filter(interview => {
    ...
    switch (filterCriteria) {
      ...
      default:
        return true; // 'all' or any other criteria
    }
  });
  const [outsourceFilterCriteria, setOutsourceFilterCriteria] = useState('all'); // New state for filter criteria
  ...
  const [taskFilterCriteria, setTaskFilterCriteria] = useState('all'); // New state for filter criteria
  ...
  const [notificationFilterCriteria, setNotificationFilterCriteria] = useState('all'); // New state for filter criteria
  ...
  return (
    <>
      <div className="lg:mx-auto lg:container xl:mx-auto xl:container 2xl:mx-auto 2xl:container mb-5">
        <div className="py-3 sm:py-0 mx-2 sm:mx-0 md:mx-0">
          <div className="grid grid-cols-9 gap-1 mx-3 sm:mx-0 md:mx-0">
            {/* left column */}
            ...
            {/* Middle content */}
            <div className="col-span-5 sm:col-span-9 md:col-span-9 space-y-4 p-2 sm:bg-white bg-[#217989] bg-opacity-5 h-[200vh] -mt-8 pt-11 -mb-8 -ml-5 sm:ml-0 -mr-1 sm:mr-0">
                       ...
                       interview.candidate?.Gender === "Male" ? (
                         ...
                         <img src={genderlessImage} alt="Other Avatar" className="w-12 h-12" />
                            ...
                            <div>
                              ...
                              <p>{displayDateTime(round.dateTime)}</p>
                            </div>
                          </div>
                        </div>
                      );
